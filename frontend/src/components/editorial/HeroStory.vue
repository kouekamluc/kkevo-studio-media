<template>
  <article v-if="story" class="relative group bg-white dark:bg-gradient-to-br dark:from-[#0D1524] dark:to-[#070B13] border border-slate-200 dark:border-white/[0.08] dark:hover:border-kkevo-blue/40 rounded-2xl overflow-hidden transition-all duration-300 shadow-md hover:shadow-xl dark:shadow-[0_12px_40px_-10px_rgba(0,0,0,0.9)]">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-0">
      <!-- Media Column -->
      <div class="lg:col-span-7 relative overflow-hidden bg-slate-900 aspect-[16/10] lg:aspect-auto min-h-[320px] lg:min-h-[480px]">
        <img
          ...
          loading="eager"
        />
        <div class="absolute inset-0 bg-gradient-to-t from-[#06090E]/90 via-black/20 to-transparent lg:opacity-60"></div>
        ...
        <div v-if="story.hero_caption" class="absolute bottom-3 left-4 right-4 hidden sm:block text-[11px] text-slate-300 bg-black/80 px-3 py-1.5 rounded-md backdrop-blur-md max-w-xl border border-white/10">
          {{ story.hero_caption }} <span v-if="story.hero_credit" class="text-slate-400 font-mono">({{ story.hero_credit }})</span>
        </div>
      </div>

      <!-- Content Column -->
      <div class="lg:col-span-5 p-6 sm:p-8 lg:p-10 flex flex-col justify-between space-y-6">
        <div class="space-y-4">
          ...
          <router-link :to="`/article/${story.slug}`" class="block group/title">
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 dark:text-white leading-[1.18] font-headline group-hover/title:text-kkevo-green transition-colors">
              {{ story.title }}
            </h1>
          </router-link>

          <!-- Dek / Subtitle -->
          <p v-if="story.subtitle" class="text-sm sm:text-base text-slate-600 dark:text-slate-200 leading-relaxed font-normal">
            {{ story.subtitle }}
          </p>
        </div>

        <!-- Metadata & Author Byline -->
        <div class="pt-5 border-t border-slate-200 dark:border-white/[0.08] flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          <div class="flex items-center gap-3">
            <img
              v-if="story.authors?.[0]?.effective_avatar"
              :src="story.authors[0].effective_avatar"
              :alt="story.authors[0].display_name"
              class="w-10 h-10 rounded-full object-cover border-2 border-kkevo-green/50 shadow-sm"
            />
            <div>
              <span class="font-bold text-slate-900 dark:text-white block text-sm">{{ story.authors?.[0]?.display_name || 'KKEVO Newsroom' }}</span>
          ...
          <button
            ...
            :class="[
              ...
              authStore.isBookmarked(story.id)
                ...
                : 'bg-slate-100 dark:bg-[#111A2B] border-slate-200 dark:border-white/[0.08] text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'
            ]"
            title="Save Story"
          >
